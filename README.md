...
| ✅ **LLMs in production** | NVIDIA NIM Llama 3.1 + NEVA 22B live inside a Django platform |
| ✅ **Measured ML results** | Precision@10: 0.345 · +47% uplift over baseline — numbers, not claims |
| ✅ **Production infra** | Nginx + Daphne (ASGI) + Gunicorn (WSGI) — not `python manage.py runserver` |
| ✅ **Real-time systems** | Django Channels + Redis + WebSockets — live messaging in deployment |
| ✅ **Payment integration** | Razorpay with webhook verification + invoice generation end-to-end |
...
`Python` `Django` `DRF` `PostgreSQL` `Redis` `WebSockets` `NVIDIA NIM` `Razorpay` `Telegram Bot API`
...
| **Context-Aware Hybrid** | **0.345** | **0.289** | **63.4%** |
| **Uplift** | **+47% ✅** | — | — |

`Scikit-learn` `HuggingFace` `DistilBERT` `FastAPI` `PostgreSQL` `Power BI` `Spotify API`
...
`Python` `Pandas` `NumPy` `SciPy` `SQL` `Power BI`
...
LLM-powered recipe generation with image-based ingredient detection, nutrition-aware recommendations, BPE-optimised prompting, and bilingual guidance — served via FastAPI with structured MySQL storage.

`Python` `FastAPI` `MySQL` `LLMs` `Computer Vision`
...
| ML Engineer | Fresher · 0–1 yr | ✅ Open |
| AI Engineer | Fresher · 0–1 yr | ✅ Open |
